...
class Visualiser():
    ...
    def plot_line(self, x, y, key, split_name):
        if key not in self.error_plots:
            ...
            #np.column_stack((x, y))
            #self.vis.line(X=np.array([x]), Y=np.array([y]), win=self.error_plots[key], name=split_name)
            
            #self.vis.scatter(np.column_stack((np.array([x]), np.array([y]))), win=self.error_plots[key],legend=[split_name],update='append')
            
    # errors: dictionary of error labels and values
    def plot_current_errors(self, epoch, error, counter_ratio=0.0):
        #if self.display_id > 0:
            #for key in errors.keys():
                #x = epoch + counter_ratio
                #y = errors[key]
                #print('dsfdsgffffffffffffffffffffffffffffffffffffffffffffffff00000',split_name,y)
                #if isinstance(y, dict):
                #    if y['type'] == 'table':
                        #self.plot_table_html(x,y,key,split_name, **kwargs)
                print('dsfdsgffffffffffffffffffffffffffffffffffffffffffffffff111111111111',split_name)
                #elif np.isscalar(y):                                         
               # self.plot_line(epoch,error)
                #    print('dsfdsgffffffffffffffffffffffffffffffffffffffffffffffff22222222222',split_name,key)
                #elif y.ndim == 2:
                #    self.plot_heatmap(x,y,key,split_name, **kwargs)
                #    print('dsfdsgffffffffffffffffffffffffffffffffffffffffffffffff333333333',split_name)
    ...
    def save_images(self, webpage, visuals, image_path):
        ...
        for label, image_numpy in visuals.items():
            ...
            print('hiiiiiiiiiiiiiiiiiiiiiiiiiiii222222')
